...
def listTwinings(client_id, dbParams):
    ...
    c.execute("SELECT a1.user_login AS login1, a1.user_email AS email1, a1.id AS id1, a2.user_login AS login2, a2.user_email AS email2, a2.id AS id2 FROM wp_twinings INNER JOIN wp_users a1 ON a1.id = wp_twinings.academy_1 INNER JOIN wp_users a2 ON a2.id = wp_twinings.academy_2 WHERE a1.user_email = %s OR a2.user_email = %s ", client)
    twlist = c.fetchall()
    return twlist 

def errorRoutine(writable, message):
    jsonError = { "error":True, "message":message}
    writable.write(json.dumps(jsonError))
